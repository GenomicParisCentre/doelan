 ...
 */
public class GlobalMaxBadFeatureRemoved extends QualityGlobalTest implements
    Module {
  ...
  public AboutModule aboutModule() {
    ...
    if (aboutModule == null) {
      ...
      try {
        md = new ModuleDescription("GlobalMaxBadFeaturesRemovedTest",
            "Count the maximal bad spots removed in a test suite");
  ...
  protected QualityGlobalTestResult test(final BioAssay bioassay,
    ...
    try {
      ...
      for (int i = 0; i < unitResults.length; i++) {
        if (unitResults[i].isFilterFlags()
            && unitResults[i].getNewFlags() != null)
          countArrays++;
      }
      ...
      for (int i = 0; i < unitResults.length; i++) {
        ...
        if (unitResults[i].isFilterFlags() && nf != null)
   ...
   *                 <b>null </b>.
   */
  public GlobalMaxBadFeatureRemoved() throws PlatformException {
    // MUST BE EMPTY
  }
