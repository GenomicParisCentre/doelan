 ...
 */
public class MaxDiameterFeatureTest extends QualityUnitTest implements Module {
  ...
  public AboutModule aboutModule() {
    ...
    if (aboutModule == null) {
      ...
      try {
        md = new ModuleDescription("MaxDiameterFeatureTest",
            "Test maximal diameter of features");
  ...
  protected Parameters defineParameters() {
    ...
    try {
      ...
      final Parameter max = new ParameterBuilder().withName("max")
          .withLongName("Maximal diameter value").withType(
              Parameter.DATATYPE_DOUBLE).withDescription(
  ...
  public QualityUnitTestResult test(final BioAssay bioassay,
    ...
    try {
      final double max = parameters.getParameter("max").getDoubleValue();
   ...
   *           <b>null </b>.
   */
  public MaxDiameterFeatureTest() throws PlatformException {
    // MUST BE EMPTY
  }
